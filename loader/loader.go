...
func LoadRootsWithConfig(cfg *packages.Config, roots ...string) ([]*Package, error) {
	...
	l.cfg.Mode |= packages.NeedName | packages.NeedFiles | packages.NeedCompiledGoFiles | packages.NeedImports | packages.NeedTypesSizes | packages.NeedTypes
	if l.cfg.Fset == nil {
	...
	addNestedGoModulesToRoots := func(p string, d os.DirEntry, e error) error {
		if e != nil {
			return e
		}
